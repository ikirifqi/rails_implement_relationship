...
article_titles = ['Matahari terbenam ',
  'Bandung berawan',
...
article_titles.each do |title|
  article_ids << Article.create(:title => title, :body => article_body).id
end

puts 'Seeding comments...'
comments = ['artikelnya bagus kk',
  'izin kopas gan',
  'izin sedot gan',
  'gan, ane ninggalin jejak dulu',
  'ane pernah denger tuh',
  'artikel ini repost',
  'PM sent gan',
  'cek PM agan ganteng',
  'terima kasih mastah',
  'ajarin ane gan, pliss']

50.times do |index|
  Comment.create(:article_id => article_ids.sample, :body => comments.sample )
end
